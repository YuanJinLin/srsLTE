...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
	...
	("expert.x2ap_myaddr",   bpo::value<string>(&args->stack.x2ap_myaddr)->default_value("192.168.0.1"),"IP address to bind X2AP connection")
	("expert.x2ap_neiaddr",   bpo::value<string>(&args->stack.x2ap_neiaddr)->default_value("192.168.0.2"),"IP address to bind X2AP connection")
  ...
  if (vm.count("log.all_level")) {
    ...
    if (!vm.count("log.x2ap_level")){
        args->stack.log.x2ap_level = args->log.all_level;
    }
  ...
  if (vm.count("log.all_hex_limit")) {
    ...
    if (!vm.count("log.x2ap_hex_limit")) {
      args->stack.log.x2ap_hex_limit = args->log.all_hex_limit;
    }
